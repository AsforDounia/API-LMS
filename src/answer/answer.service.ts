...
export class AnswerService {
  ...
  async create(createAnswerDto: CreateAnswerDto): Promise<Answer> {
    ...
    const attempt = await this.quizAttemptModel.findById(createAnswerDto.attemptId);
    ...
    if (alreadyAnswered) {
      throw new BadRequestException('You have already answered this question in this attempt.');
    }
    // Vérification de la progression (verrouillage séquentiel)
    // Récupérer le quiz et le module associé
    const quiz = await this.quizModel.findById(attempt.quizId).populate('moduleId');
    ...
    const isCorrect =
      question.correctAnswers.length === createAnswerDto.selectedAnswers.length &&
      question.correctAnswers.every((ans) => createAnswerDto.selectedAnswers.includes(ans));
  ...
  async submitQuiz(submitQuizDto: SubmitQuizDto): Promise<{ success: boolean; message: string; results: { questionId: string; status: string; error?: any }[] }> {
    ...
    for (const answerDto of answers) {
      try {
        ...
        results.push({ questionId: answerDto.questionId, status: 'error', error: e.message });
      }
    }
    return {
      success: true,
